...
function opacityForCell(id: string, phase: number): number {
	...
	const pathIndex = PERIMETER_PATH.indexOf(
		id as (typeof PERIMETER_PATH)[number],
	);
...
export function DotmHex1({
	...
	const gap =
		...
		Math.max(
			1,
			Math.floor((size - dotSize * ROW_COUNTS[2]) / (ROW_COUNTS[2] - 1)),
		);
	...
	const matrixHeight =
		dotSize * ROW_COUNTS.length + rowGap * (ROW_COUNTS.length - 1);
				...
				{ROW_COUNTS.map((count, row) => (
					...
					>
						{Array.from({ length: count }).map((_, col) => {
							const id = `${row},${col}`;
							const isActive = activePatternIndexes.includes(
								hexPatternIndex(row, count, col),
							);
